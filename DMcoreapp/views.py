...
def ad_follow_tele_det(request,id):
    if request.session.has_key('userid'):
        ids = request.session['userid']
    else:
        return redirect('/')
...
def ad_delay_flt(request):
    ...
    if request.method=="POST":
        ...
        dl_task=lead_delay.objects.filter(date__gte=st_dt, date__lte=end_dt, executive_id=exe)
        context = {
            "usr":usr,
            'dl_task': dl_task,
            "exe":exes
...
def tc_view_current_leads(request):
    ...
    cur_leads = All_leads.objects.filter(Q(telecaller_id=ids) &
                
                ( Q(followup_dt__lte=date.today()) & (Q(status='Follow up') |
                Q(status='Interested') |Q(status='Want to visit Office') |Q(status='Will inform') |Q(status='Payment issue') |
                Q(status='Please send details through WhatsApp') | Q(status='Join later')| Q(status='Will contact after enquiry'))) |
                
                ( Q(followup_dt= None) & (Q(status='Follow up') | 
                Q(status='Interested') |Q(status='Want to visit Office') |Q(status='Will inform') |Q(status='Payment issue') |
                Q(status='Please send details through WhatsApp') | Q(status='Join later')| Q(status='Will contact after enquiry') ))|
                
                (Q(assign_dt__lte=date.today()) | (Q(status='Follow up') |
                Q(status='Interested') |Q(status='Want to visit Office') |Q(status='Will inform') |Q(status='Payment issue') |
                Q(status='Please send details through WhatsApp') | Q(status='Join later')| Q(status='Will contact after enquiry')))|

                (Q(followup_dt= None) & Q(status=None)))
    ...
    #             (Q(assign_dt=date.today()) &(Q(status='Follow up') |
    #             Q(status='Interested') |Q(status='Want to visit Office') |Q(status='Will inform') |Q(status='Payment issue') |
    #             Q(status='Please send details through WhatsApp') | Q(status='Join later')| Q(status='Will contact after enquiry')))|
...
def tc_view_previous_leads(request):
    ...
    caller = user_registration.objects.get(id=callerid)
    # print(callerid)
    pre_leads = All_leads.objects.filter(telecaller_id=ids)
    count = All_leads.objects.filter(
            Q(telecaller_id=ids, followup_dt=date.today()) &
            (Q(status='Interested') | Q(status='Want to visit office') | Q(status='Will inform') | Q(status='Will contact if interested') | 
            Q(status='Join later') | Q(status='Call you back') | Q(status='Will contact after enquiry') | Q(status='Follow up'))).count()
...
def tc_view_notifications(request):
    ...
    Q(telecaller_id=ids, followup_dt=date.today()) &
    (Q(status='Interested') | Q(status='Want to visit office') | Q(status='Will inform') | Q(status='Will contact if interested') | Q(status='Join later') | Q(status='Call you back') | Q(status='Will contact after enquiry'))
)
    # print('hi')
    # for i in cur_leads:
    #     print(i)
...
def dm_telecallers(request):
    ...
    if request.session.has_key('userid'):
        ...
        context={'telecallers': telecallers,
                 'usr':usr}
        return render(request, 'dm_manager/dm_telecallers.html',context)
    else:
        return redirect('/')
#replace

def dm_followup(request,t_id):
    if request.session.has_key('userid'):
        ids = request.session['userid']
    else:
        return redirect('/')
    ...
    usr = user_registration.objects.get(id=ids)
    status_values=['Will inform','Interested','Want to visit office', 'Please send details through WhatsApp','Join later','Will contact after enquiry','Will contact if interested','Follow up']
    followup=All_leads.objects.filter(telecaller_id = t_id,status__in=status_values).order_by('-followup_dt')
    # print("hi")
    # print(t_id)
    context={
        "usr":usr, 
